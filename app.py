...
def get_client():
    # Prefer an environment/Streamlit secret when available; otherwise use the sidebar key.
    key = os.getenv("GEMINI_API_KEY", "").strip() or st.session_state.api_key.strip()
    # Avoid treating accidental copied quotes/backticks as part of the API key.
    ...
    try:
        ...
        message = str(exc)
        if "API_KEY_INVALID" in message or "API key not valid" in message or "authentication" in message.lower():
            st.error(
                "Gemini API Key ditolak. Gunakan API key AUTH baru dari Google AI Studio "
                "(Key Type: Auth), lalu masukkan ulang. Jangan kirim key ke chat."
            )
        else:
    ...
    for attempt in range(4):
        try:
            ...
            if "API_KEY_INVALID" in message or "API key not valid" in message:
                raise RuntimeError(
                    "Gemini API Key ditolak. Buat/copy API key AUTH baru di Google AI Studio "
                    "(Key Type: Auth), lalu ganti key di aplikasi. Ini bukan error limit."
                ) from exc
...
def file_parts(client, uploaded_file):
    ...
    if not remote:
        return []
    # Gemini Files API returns a File resource. Content.parts must receive a
    # Part, not the File resource itself. Convert the uploaded media to the
    # canonical URI Part representation.
