...
from validation_core import (
    ...
    validate_scene_coverage, validate_scene_transition,
    validate_causal_integrity, validate_prompt_against_contract,
)

st.set_page_config(page_title="UGC Remix Studio V5.15", page_icon="🎬", layout="wide")

MODEL_NAME = "gemini-3.6-flash"
APP_VERSION = "5.15 RC — Evidence Reconciliation + State Continuity + Causal Gates + Hard Bridge"
...
PERFORMANCE_PROFILE_SCHEMA = {
    ...
    "required": ["subject_id", "baseline_emotion", "emotional_role", "trigger_map", "arc", "facial_performance", "head_and_gaze", "body_performance", "movement_quality", "micro_reactions", "attention_targets", "timing_logic"],
}
...
REFERENCE_READABILITY_SCHEMA = {
    ...
    "properties": {
        ...
        "subject_roster": SUBJECT_ROSTER_SCHEMA,
    },
...
def cached_reference_video_part(client, uploaded_file):
    ...
    if fallback is not None:
        ...
        st.session_state.reference_input_mode = "inline"
        st.session_state.reference_processing_error = None
        st.warning("Files API gagal memproses video. Sistem beralih ke Inline Video karena ukuran file di bawah 20 MB.")
        return [fallback]
    return []
...
def normalize_subject_roster(data: dict) -> tuple[dict, list[dict]]:
    """Normalize structured subject output without inventing subjects.

    The validator accepts only subject objects that contain a stable id. Optional
    descriptive fields are defaulted locally so a partially populated structured
    response does not crash downstream continuity logic. No subject is fabricated.
    """
    roster = data.get("subject_roster")
    repair_log = []
    ...
    if isinstance(roster, list):
        ...
        candidate = None
        for key in ("subjects", "subjek", "items", "data", "detected_subjects", "detectedSubjects"):
        ...
        if candidate is not None:
            ...
            repair_log.append({"field": "subject_roster", "reason": "object_wrapper", "action": "unwrapped_subject_list"})
        elif roster.get("id") or roster.get("jenis") or roster.get("deskripsi_visual"):
            normalized = [roster]
            repair_log.append({"field": "subject_roster", "reason": "single_subject_object", "action": "wrapped_as_list"})
        else:
            normalized = []
    else:
        normalized = []
    ...
    for item in normalized:
        ...
        item = dict(item)
        item["id"] = sid
        item.setdefault("jenis", "subjek")
        item.setdefault("peran", "")
        item.setdefault("deskripsi_visual", "")
        item.setdefault("posisi_awal", "")
        item.setdefault("orientasi", "")
        item.setdefault("ekspresi_emosi", "")
        item.setdefault("aksi_perilaku", "")
        item.setdefault("interaksi_dengan", [])
        item.setdefault("fungsi_dalam_humor_atau_emosi", "")
        item.setdefault("wajib_terlihat", True)
        if not isinstance(item.get("interaksi_dengan"), list):
            item["interaksi_dengan"] = [str(item["interaksi_dengan"])] if item["interaksi_dengan"] else []
        cleaned.append(item)

    if not cleaned:
        raise ValueError(
            "Gemini berhasil menerima permintaan tetapi tidak memberikan daftar subjek terdeteksi. "
            "Video belum bisa dianggap terbaca untuk analisis reference. "
            "Sistem tidak akan mengarang subject_roster."
        )

    data["subject_roster"] = cleaned
    return data, repair_log
...
"""
    ...
    probe_data = {"subject_roster": result.get("subject_roster", [])}
    probe_data, _ = normalize_subject_roster(probe_data)
    return probe_data["subject_roster"]
...
def _validate_reference_pipeline(data: dict):
    ...
    fidelity = data.get("reference_fidelity") or {}
    req = ("camera_geometry", "subject_geography", "motion_physics_map", "action_critical_props", "hook_mechanics", "payoff_fidelity")
    issues = [k for k in req if not str(fidelity.get(k) or "").strip()]
    if not isinstance(fidelity.get("must_preserve"), list) or not fidelity.get("must_preserve"): issues.append("must_preserve")
    if not isinstance(fidelity.get("forbidden_drift"), list) or not fidelity.get("forbidden_drift"): issues.append("forbidden_drift")
    fidelity_result = {"ok": not issues, "issues": [f"reference_fidelity tidak substantif: {x}" for x in issues]}
    _record_validation("reference_fidelity_semantic", fidelity_result)
    if not fidelity_result["ok"]:
        raise ValueError("Reference fidelity semantic validation gagal: " + ", ".join(issues))
    coverage = data.get("scene_coverage_map") or []
    coverage_result = {"ok": isinstance(coverage, list) and bool(coverage), "issues": [] if coverage else ["scene_coverage_map kosong"]}
    _record_validation("scene_coverage_map", coverage_result)
    if not coverage_result["ok"]:
        raise ValueError("scene_coverage_map kosong; mapping temporal → scene wajib tersedia.")
    return True
...
"""
    ...
    with st.spinner("Membedah referensi penuh: timeline, seat map, kamera, geography, motion physics, prop state, performance, hook, dan payoff..."):
        try:
            ...
            try:
                data, roster_repair_log = normalize_subject_roster(data)
            except ValueError:
                # The readability gate already observed real subjects in the same
                # multimodal request. Reuse that evidence as a structural seed rather
                # than inventing subjects or aborting the entire analysis.
                data["subject_roster"] = st.session_state.get("reference_readability_subjects", [])
                if not data["subject_roster"]:
                    raise
                data, roster_repair_log = normalize_subject_roster(data)
                roster_repair_log.append({"field": "subject_roster", "reason": "full_analysis_missing_but_probe_valid", "action": "used_readability_probe_roster"})
            if roster_repair_log:
                st.warning(
                    "⚠️ Struktur subject_roster dari Gemini tidak langsung berupa list. "
                    "Sistem menormalkan wrapper JSON tanpa menambah subjek baru. "
                    f"Repair: {len(roster_repair_log)} item."
                )
            emotion_data = data.get("emotion_performance_analysis")
            if not isinstance(emotion_data, dict):
                raise ValueError("emotion_performance_analysis harus berupa object performance yang lengkap.")
            ...
            repaired_ids = {str(x.get("subject_id")) for x in repair_log if isinstance(x, dict) and x.get("subject_id")}
            st.session_state["performance_provenance"] = {sid: ("LOCAL_FALLBACK" if sid in repaired_ids else "MODEL_VERIFIED") for sid in subject_ids}
            if repair_log:
                st.info(
                    "ℹ️ Gemini tidak mengisi semua performance profile secara lengkap. "
