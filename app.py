...
10. Jika ada DURATION EXTENSION, gunakan duration_extension_plan dari analisis untuk mengisi selisih waktu dengan micro-action yang aman, lucu/menarik/penasaran, tetap berada di lokasi dan geografi yang sama, serta mempunyai sebab-akibat jelas. Micro-action harus menjadi jembatan, bukan filler acak.
11. Jika tidak ada DURATION EXTENSION dan jumlah target scene lebih besar daripada jumlah beat referensi, pecah satu beat menjadi beberapa langkah mikro yang memang terjadi secara natural; jangan membuat plot baru.
12. Jika jumlah target scene lebih kecil, gabungkan beat yang berdekatan tanpa menghilangkan sebab-akibat.
...
def render_scenes():
    ...
    # The storyboard duration is locked when the storyboard is created.
    # This prevents a changed duration selector from silently changing the scene flow.
    if st.session_state.storyboard_duration != st.session_state.duration or n != expected:
        locked = st.session_state.storyboard_duration or "tidak diketahui"
        st.error(
            f"Jumlah scene tidak sesuai: storyboard = {n} scene, "
            f"durasi aktif = {st.session_state.duration} = {expected} scene."
        )
        st.info(
            f"Storyboard ini terkunci pada durasi {locked}. "
            "Buat ulang storyboard supaya urutan scene kembali sinkron."
        )
        if st.button("BUAT ULANG STORYBOARD", type="primary", use_container_width=True):
            run_storyboard()
        return
    ...
    # Clear progress: Scene X of N, not X/1 unless the project really has one scene.
    st.progress(current / n)
    st.markdown(f"### SCENE {current} / {n}")
    st.caption(
        f"Durasi proyek: {st.session_state.duration} | "
        f"Setiap scene: sekitar 8 detik | "
        "Alur scene dikunci berurutan."
    )
    ...
    # Continuity gate: every scene after Scene 1 must use the last frame of the previous scene.
    previous_frame = None
    if current > 1:
        st.divider()
        st.subheader(f"Continuity Scene {current - 1} -> Scene {current}")
        st.info(
            f"1. Generate Scene {current - 1} di Flow/Veo. "
            f"2. Ambil screenshot frame terakhir. "
            f"3. Upload screenshot tersebut di bawah. "
            f"4. Baru buat prompt Scene {current}."
        )
        previous_frame = st.file_uploader(
            f"Upload screenshot frame terakhir Scene {current - 1}",
            type=["png", "jpg", "jpeg", "webp"],
            key=f"frame_{current}",
        )
        if previous_frame is not None:
            st.session_state.scene_frames[current - 1] = previous_frame
            st.success(f"Frame terakhir Scene {current - 1} tersimpan dan siap dipakai.")
        else:
            previous_frame = st.session_state.scene_frames.get(current - 1)
            if previous_frame is not None:
                st.success(f"Frame terakhir Scene {current - 1} sudah tersimpan.")

    st.divider()

    # Prompt generation is deliberately separated from navigation.
    # This makes the workflow explicit: generate -> make video -> save last frame -> next scene.
    if current not in st.session_state.scene_prompts:
        can_generate = current == 1 or previous_frame is not None
        if current > 1 and not can_generate:
            st.warning(
                f"Scene {current} belum bisa dibuat. Upload dulu screenshot frame terakhir Scene {current - 1}."
            )
        if st.button(
        ...
        st.info(
            f"WORKFLOW: Generate video Scene {current} di Flow/Veo -> "
            "ambil screenshot frame terakhir -> kembali ke sini -> "
        ...
        with right:
            if current < n:
                # HARD GATE: the next scene cannot be opened until the last
                # frame of the current generated scene has been uploaded.
                next_frame = st.file_uploader(
                    f"Upload screenshot frame TERAKHIR Scene {current} sebelum lanjut",
                    type=["png", "jpg", "jpeg", "webp"],
                    key=f"next_frame_{current}",
                    help="Ambil screenshot dari frame paling akhir video Scene saat ini.",
                )
                if next_frame is not None:
                    st.session_state.scene_frames[current] = next_frame
                    st.success(
                        f"Screenshot akhir Scene {current} sudah tersimpan. "
                        f"Sekarang Scene {current + 1} bisa dibuka."
                    )
                else:
                    next_frame = st.session_state.scene_frames.get(current)

                can_next = next_frame is not None
                if st.button(
                    f"SELESAI SCENE {current} → BUKA SCENE {current + 1}",
                    type="primary",
                    use_container_width=True,
                ...
                if not can_next:
                    st.caption(
                        f"🔒 Scene {current + 1} terkunci. "
                        f"Upload screenshot frame terakhir Scene {current} terlebih dahulu."
                    )
            else:
                if st.button(
                    "SEMUA SCENE SELESAI - LANJUT KE SEO",
                    type="primary",
                    use_container_width=True,
                ):
                    go("seo")

    # No free scene jumping. The project must move 1 → 2 → 3 → ... only.
