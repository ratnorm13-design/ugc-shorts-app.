...
st.set_page_config(page_title="UGC Reference Studio", page_icon="ðŸŽ¬", layout="wide")
...
with st.sidebar:
    ...
    st.caption("Referensi â†’ Continuity Analysis â†’ Storyboard â†’ Flow/Veo")
...
# ============================================================
# REFERENCE ANALYSIS â€” TEMPORAL + SPATIAL + STATE
# ============================================================
def run_analysis():
    client = get_client()
    if not client:
...
def render_scenes():
    ...
    if current not in st.session_state.scene_prompts:
        ...
        with right:
            if current < n:
                if st.button(
                    f"SELESAI SCENE {current} - LANJUT KE SCENE {current + 1}",
                    type="primary",
                    use_container_width=True,
                ):
                    st.session_state.current_scene = current + 1
                ...
                if st.button("SEMUA SCENE SELESAI - LANJUT KE SEO", type="primary", use_container_width=True):
                    go("seo")

    st.divider()
    st.subheader("Navigasi Scene")
    jump = st.selectbox(
        "Pilih scene",
        list(range(1, n + 1)),
        index=current - 1,
