...
def upload_to_gemini(client, uploaded_file):
    """Upload reference media through Gemini Files API and return the File resource."""
    if uploaded_file is None:
        return None
    try:
        mime = getattr(uploaded_file, "type", None) or "application/octet-stream"
        display_name = getattr(uploaded_file, "name", "reference") or "reference"
...
def file_parts(client, uploaded_file):
    """Return only valid google.genai.types.Part objects for Content.parts."""
    if uploaded_file is None:
        return []
    ...
    # IMPORTANT: client.files.upload() returns a File resource, not a
    # Content.parts item. Because ask() constructs types.Content explicitly,
    # the File must be converted to a URI Part first. Passing the File object
    # directly here can serialize as an empty Part and produce:
    # "parts[1].data: required one of field 'data'..."
    remote = upload_to_gemini(client, uploaded_file)
    if not remote:
        return []
    uri = getattr(remote, "uri", None)
    remote_mime = getattr(remote, "mime_type", None) or mime
    if not uri:
        raise RuntimeError("Gemini Files API tidak mengembalikan URI file referensi.")
    return [types.Part.from_uri(file_uri=uri, mime_type=remote_mime)]
...
def reference_parts(client):
    ...
    if ref_type == "Teks / ide" and st.session_state.reference_text.strip():
        return [types.Part.from_text(text=f"REFERENSI TEKS PENGGUNA:\n{st.session_state.reference_text}")]
    return []
