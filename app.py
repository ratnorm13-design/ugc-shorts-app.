...
3. DURATION DETECTION: Tentukan durasi asli video referensi dalam detik.
...
{{
  ...
  "storyboard_plan": [
    {{"scene": 1, "fokus_aksi": "Runner meluncur dengan akselerasi kaget, menendang Target A jatuh dari dudukan."}},
    {{"scene": 2, "fokus_aksi": "Runner melakukan manuver parkour konyol melewati rintangan, menendang Target B."}}
  ],
...
def render_home():
    ...
    st.subheader("2. Pilihan Karakter Runner & Target Ragdoll")
    ...
    with col_k1:
        ...
        if st.session_state.runner_choice == "Custom / Ketik Sendiri":
            st.text_input("Tulis Deskripsi Karakter Bebas Kamu:", key="custom_runner")
    with col_k2:
        st.selectbox("🎯 Pilih Karakter Target / Ragdoll:", TARGET_DOLL_PRESETS, key="target_doll_choice")
    with col_k3:
        st.selectbox("🎭 Gaya Gerakan Idle Target:", TARGET_IDLE_PRESETS, key="target_idle_choice")
    ...
    st.subheader("4. Pengaturan Visual, Kamera, Rasio Aspek & Target Durasi")
    col1, col2 = st.columns(2)
    with col1:
        st.selectbox("🎨 Gaya Visual", STYLE_OPTIONS, key="visual_style")
        st.selectbox("📷 Pergerakan Kamera:", CAMERA_OPTIONS, key="selected_camera")
    ...
    with col2:
        ...
        st.text_area("📝 Instruksi Tambahan (Opsional)", key="custom_instruction", height=80)

    st.markdown("---")
    st.subheader("🧭 Navigasi Rintangan & Manuver Parkour Per-Scene")
    ...
    for i in range(1, calculated_scenes + 1):
        ...
        with col_s1:
            chosen_obs = st.selectbox(f"Scene {i} Rintangan Track:", options=list(OBSTACLE_OPTIONS.keys()), index=0, key=f"obs_{i}")
            st.session_state.user_scene_obstacles[i] = OBSTACLE_OPTIONS[chosen_obs]
        with col_s2:
            chosen_man = st.selectbox(f"Scene {i} Manuver Parkour:", options=MANEUVER_OPTIONS, index=0, key=f"man_{i}")
...
def render_scenes():
    st.title("🎥 Flow AI Video Prompt Generator")
    n = scene_count()
    ...
    if current in st.session_state.scene_prompts:
        st.text_area("Prompt AI Video (Copy-Paste ke Flow AI):", value=st.session_state.scene_prompts[current], height=200)
