...
def detect_video_duration(uploaded_file):
    """Best-effort duration detection for uploaded reference videos."""
...
def choose_target_duration(seconds):
    """Choose the smallest supported target duration that can contain the reference."""
    if seconds is None:
        return None
    options = list(DURATION_SCENES.keys())
    duration_seconds = {
        "8 detik": 8, "16 detik": 16, "24 detik": 24, "32 detik": 32,
        "40 detik": 40, "48 detik": 48, "56 detik": 56, "1 menit": 60,
        "1,5 menit": 90, "2 menit": 120, "2,5 menit": 150, "3 menit": 180,
        "3,5 menit": 210, "4 menit": 240, "4,5 menit": 270, "5 menit": 300,
    }
    for option in options:
        if seconds <= duration_seconds[option]:
            return option
    return "5 menit"
...
def render_home():
    ...
    if ref_type == "Video":
        ...
        st.session_state.reference_file = uploaded
        st.session_state.reference_files = []
        if uploaded is not None:
            detected = detect_video_duration(uploaded)
            st.session_state.reference_duration_seconds = detected
            target = choose_target_duration(detected)
            if target and detected is not None and target != st.session_state.duration:
                duration_seconds = {
                    "8 detik": 8, "16 detik": 16, "24 detik": 24, "32 detik": 32,
                    "40 detik": 40, "48 detik": 48, "56 detik": 56, "1 menit": 60,
                    "1,5 menit": 90, "2 menit": 120, "2,5 menit": 150, "3 menit": 180,
                ...
                if detected > duration_seconds.get(st.session_state.duration, 0):
                    st.session_state.duration = target
                    st.session_state.duration_auto_adjusted = True
            if detected is not None:
                st.info(f"Durasi referensi terdeteksi: {detected:g} detik → target produksi: {st.session_state.duration}.")
                if detected < {
                    "8 detik":8,"16 detik":16,"24 detik":24,"32 detik":32,"40 detik":40,"48 detik":48,
                    "56 detik":56,"1 menit":60,"1,5 menit":90,"2 menit":120,"2,5 menit":150,"3 menit":180,
                    "3,5 menit":210,"4 menit":240,"4,5 menit":270,"5 menit":300
                }[st.session_state.duration]:
                    st.success("AI akan mengisi selisih durasi dengan micro-action yang tetap nyambung, bukan filler random.")
...
12. Jika durasi referensi lebih pendek dari target produksi, pertahankan seluruh kejadian utama lalu isi selisih durasi dengan micro-action yang masuk akal: reaksi Milo, rasa penasaran, antisipasi, komedi kecil, atau payoff yang diperjelas. Micro-action harus punya hubungan sebab-akibat dengan kejadian terdekat dan tidak boleh berupa filler random.
13. Untuk karakter utama, gunakan Milo sebagai karakter Tale Of Paw. Karakter pendukung hanya boleh ada jika memang diperlukan oleh referensi/alur.
14. Properti biasa dan lingkungan yang tidak berbahaya harus dipertahankan secara visual. Untuk elemen berbahaya, pertahankan fungsi dramatis/komedinya tanpa detail operasional dan buat representasinya aman.
15. Jangan mengubah lokasi atau geografi hanya agar prompt terlihat lebih sinematik.
16. Semua nilai JSON berbahasa Indonesia.
...
Durasi referensi terdeteksi: {st.session_state.reference_duration_seconds} detik
Durasi target produksi: {st.session_state.duration}
...
13. Jika durasi referensi lebih pendek daripada target produksi, pertahankan seluruh alur utama referensi lalu tambahkan micro-action yang natural (reaksi, rasa penasaran, antisipasi, komedi kecil, atau payoff yang diperjelas). Micro-action wajib punya sebab-akibat dan tidak boleh menjadi filler random.
14. Semua teks JSON Bahasa Indonesia.
    ...
    with st.spinner(f"Menyusun {n} scene dengan continuity contract..."):
        try:
            ...
            st.session_state.duration_extension = {
                "reference_duration_seconds": st.session_state.reference_duration_seconds,
                "target_duration": st.session_state.duration,
                "note": "Tambahkan micro-action natural hanya bila target lebih panjang dari referensi."
            }
            st.session_state.current_scene = 1
...
def render_scenes():
    ...
    expected = st.session_state.target_scene_count or DURATION_SCENES.get(locked_duration, len(scenes))
    actual = len(scenes)
