...
def normalize_subject_roster(data: dict) -> tuple[dict, list[dict]]:
    """Normalize Gemini's subject roster without inventing subjects.

    Structured output should already return a list, but some Gemini responses can
    serialize a collection under an object wrapper (for example {"subjects": [...]})
    or a single detected subject as an object. Convert only those structural forms.
    If no trustworthy subject collection exists, fail loudly instead of fabricating one.
    """
    roster = data.get("subject_roster")
    repair_log = []
    ...
    # Keep only object-shaped entries with an ID; do not invent missing subjects.
    cleaned = []
    seen = set()
    ...
    if not cleaned:
        raise ValueError(
            "Gemini tidak mengembalikan subject_roster yang bisa divalidasi. "
            "Respons harus berisi list subjek terdeteksi dengan id unik. "
            "Cek apakah video benar-benar dapat dibaca Gemini."
        )
