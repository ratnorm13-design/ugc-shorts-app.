...
APP_VERSION = "5.8 — Hard Visual Continuity + Global SEO + State Hardening"
...
def run_seo():
    ...
    # First, use Gemini's Google Search grounding only for the research pass.
    # This keeps the final metadata JSON deterministic and avoids mixing tool
    # output with the app's structured JSON response.
    research_prompt = f"""
Anda adalah YouTube SEO researcher untuk channel Tale Of Paw dengan target GLOBAL.
Gunakan Google Search untuk mencari sinyal topik, istilah pencarian, dan phrasing yang benar-benar relevan
untuk video ini. Jangan mencari keyword yang tidak berhubungan hanya karena sedang populer.
...
Tugas:
1. Identifikasi 5-10 konsep pencarian yang benar-benar menggambarkan isi video.
2. Bedakan istilah broad, mid-tail, dan long-tail.
3. Cari phrasing yang lazim dipakai penonton global untuk niche ini.
4. Cari juga padanan Bahasa Indonesia yang natural, bukan terjemahan kata-per-kata yang aneh.
5. Abaikan keyword spam, clickbait yang tidak sesuai isi, dan istilah yang tidak relevan.
6. Jangan mengklaim volume pencarian atau ranking jika data tersebut tidak tersedia.

Berikan ringkasan riset singkat beserta query yang digunakan dan sumber yang paling relevan.
"""

    with st.spinner("🔎 Riset SEO + relevansi global..."):
        try:
            research = ask(client, research_prompt, google_search=True)
        except Exception as exc:
            # SEO can still work from the actual video analysis if Search grounding
            # is unavailable, but the user is explicitly told that web research failed.
            research = f"Google Search grounding gagal. Gunakan hanya analisis video yang tersedia. Error: {exc}"
            st.warning("Riset Google Search tidak tersedia; SEO tetap dibuat dari isi video agar alurnya tidak terhenti.")
...
"""
    with st.spinner("🧠 Menyusun metadata EN + ID..."):
        try:
            st.session_state.seo = extract_json(ask(client, prompt, json_mode=True))
            st.session_state.seo["research"] = research
        except Exception as exc:
