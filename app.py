...
APP_VERSION = "5.12 — Emotion & Performance + Curiosity + Hard Continuity + Global SEO"
...
19. ANALISIS PERFORMANCE, bukan hanya deskripsi. Untuk setiap subjek penting, jelaskan perubahan emosi yang terlihat, intensitas 1-5, pemicu, facial performance, body language, head/gaze, movement quality, micro-reactions, dan timing relatif terhadap aksi. Jangan menulis "terlihat panik" tanpa menjelaskan tanda visual yang membuat panik terbaca.
20. Bangun EMOTION TIMELINE yang mengikuti sebab-akibat fisik/cerita. Emosi tidak boleh berubah karena timer arbitrer; perubahan emosi harus dipicu oleh kejadian yang terlihat atau informasi yang diterima subjek.
...
"""
    ...
    with st.spinner("Membedah referensi: urutan, lokasi, kamera, karakter, properti, dan continuity..."):
        try:
            ...
            profiles = emotion_data.get("subject_emotion_profiles", [])
            if not isinstance(profiles, list):
                raise ValueError("subject_emotion_profiles harus berupa list.")
            roster_ids = {str(x.get("id")) for x in data.get("subject_roster", []) if isinstance(x, dict) and x.get("id")}
            profile_ids = {str(x.get("subject_id")) for x in profiles if isinstance(x, dict) and x.get("subject_id")}
            important_ids = {str(x.get("id")) for x in data.get("subject_roster", []) if isinstance(x, dict) and x.get("wajib_terlihat") is True and x.get("id")}
            missing_profiles = sorted(important_ids - profile_ids)
            if missing_profiles:
                raise ValueError(f"Performance profile hilang untuk subject wajib: {missing_profiles}")
            if roster_ids and not profile_ids.issubset(roster_ids):
                raise ValueError("subject_emotion_profiles mengandung subject_id yang tidak ada di subject_roster.")
