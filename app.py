...
APP_VERSION = "5.5 — Reference-First Multi-Subject Analysis"
...
def ask(client, prompt, parts=None, json_mode=False):
...
def render_home():
    ...
    if ref_type == "Video":
        ...
        if uploaded_reference is not None:
            ...
            # Filename + size is not enough: two different videos can share both.
            # Hash the actual bytes so replacing the reference always resets analysis.
...
"""
    ...
    with st.spinner("Membedah referensi: urutan, lokasi, kamera, karakter, properti, dan continuity..."):
        try:
            ...
            data.setdefault("subject_roster", [])
            data.setdefault("object_inventory", [])
            data.setdefault("spatial_layout", {})
