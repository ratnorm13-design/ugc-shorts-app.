...
from validation_core import (
    reconcile_subject_rosters, validate_temporal_crosscheck,
    validate_temporal_keyframe_alignment, validate_reference_fidelity,
    validate_performance_profiles, validate_scene_coverage,
    validate_scene_coverage_complete, validate_scene_transition,
    validate_causal_integrity, validate_prompt_against_contract,
)

st.set_page_config(page_title="UGC Remix Studio V5.17", page_icon="🎬", layout="wide")

MODEL_NAME = "gemini-3.6-flash"
APP_VERSION = "5.17 RC — Strict Evidence + No Silent Repair + State Continuity + Release Gates"
...
DEFAULTS = {
    ...
    "reference_processing_error": None,
    "reference_keyframe_timestamps": [],
    "reference_readability": {},
    "reference_readability_subjects": [],
...
def reset_project_callback():
    ...
    st.session_state["performance_provenance"] = {}
...
SUBJECT_ITEM_SCHEMA = {
    ...
    "properties": {
        ...
        "posisi_awal": {"type": "string"},
        "orientasi": {"type": "string"},
        "ekspresi_emosi": {"type": "string"},
        "aksi_perilaku": {"type": "string"},
        "interaksi_dengan": {"type": "array", "items": {"type": "string"}},
        "fungsi_dalam_humor_atau_emosi": {"type": "string"},
        "wajib_terlihat": {"type": "boolean"},
    },
    "required": [
        "id", "jenis", "peran", "deskripsi_visual", "posisi_awal", "orientasi",
        "ekspresi_emosi", "aksi_perilaku", "interaksi_dengan",
        "fungsi_dalam_humor_atau_emosi", "wajib_terlihat",
    ],
    "additionalProperties": False,
}

SUBJECT_ROSTER_SCHEMA = {"type": "array", "items": SUBJECT_ITEM_SCHEMA}

PROBE_SUBJECT_ITEM_SCHEMA = {
    "type": "object",
    "properties": {
        "id": {"type": "string"},
        "jenis": {"type": "string"},
    ...
    "required": ["id", "jenis", "deskripsi_visual"],
    "additionalProperties": False,
}
PROBE_SUBJECT_ROSTER_SCHEMA = {"type": "array", "minItems": 1, "items": PROBE_SUBJECT_ITEM_SCHEMA}

TEMPORAL_BEAT_SCHEMA = {
    "type": "object",
    "properties": {
        ...
        "payoff_relation": {"type": "string"},
        "continuity_to_next": {"type": "string"},
    },
    "required": [
        "beat", "waktu", "start_state", "cause", "action", "end_state",
        "camera_state", "character_state", "motion_state", "emotion_state"
    ],
}
TEMPORAL_BREAKDOWN_SCHEMA = {
    "type": "array",
    "minItems": 1,
    "items": TEMPORAL_BEAT_SCHEMA,
}
...
PERFORMANCE_PROFILE_SCHEMA = {
    ...
    "properties": {
        ...
        "baseline_emotion": {"type": "string"},
        "emotional_role": {"type": "string"},
        "trigger_map": {"type": "array", "items": {"type": "string"}},
        "arc": {"type": "array", "items": {"type": "string"}},
        "facial_performance": {"type": "array", "items": {"type": "string"}},
        "head_and_gaze": {"type": "array", "items": {"type": "string"}},
        "body_performance": {"type": "array", "items": {"type": "string"}},
        "hands_or_paws": {"type": "array", "items": {"type": "string"}},
        "movement_quality": {"type": "array", "items": {"type": "string"}},
        "micro_reactions": {"type": "array", "items": {"type": "string"}},
        "attention_targets": {"type": "array", "items": {"type": "string"}},
        "timing_logic": {"type": "string"},
    },
    "required": [
        "subject_id", "baseline_emotion", "emotional_role", "trigger_map", "arc",
        "facial_performance", "head_and_gaze", "body_performance", "hands_or_paws",
        "movement_quality", "micro_reactions", "attention_targets", "timing_logic",
    ],
    "additionalProperties": False,
}
EMOTION_PERFORMANCE_SCHEMA = {
    "type": "object",
    "properties": {
        "emotional_arc_global": {"type": "array", "items": {"type": "string"}},
        "performance_principles": {"type": "array", "items": {"type": "string"}},
        "reaction_hierarchy": FLEX_OBJECT_ARRAY,
        "subject_emotion_profiles": {"type": "array", "minItems": 1, "items": PERFORMANCE_PROFILE_SCHEMA},
        "emotion_timeline": FLEX_OBJECT_ARRAY,
        "curiosity_beats": FLEX_OBJECT_ARRAY,
        "remix_emotional_enhancement": FLEX_STRING_ARRAY,
        "anti_flat_performance_rules": FLEX_STRING_ARRAY,
        "scene_performance_targets": FLEX_OBJECT_ARRAY,
    },
    "required": ["emotional_arc_global", "performance_principles", "reaction_hierarchy", "subject_emotion_profiles", "emotion_timeline", "curiosity_beats", "anti_flat_performance_rules", "scene_performance_targets"],
}
...
REFERENCE_ANALYSIS_SCHEMA = {
    ...
    "properties": {
        ...
        "subject_roster": SUBJECT_ROSTER_SCHEMA, "seat_map": FLEX_OBJECT_ARRAY,
        "object_inventory": FLEX_OBJECT_ARRAY, "spatial_layout": {"type": "object"},
        "character_lock": {"type": "object"}, "prop_locks": FLEX_OBJECT_ARRAY,
        "temporal_breakdown": TEMPORAL_BREAKDOWN_SCHEMA, "urutan_kejadian": FLEX_STRING_ARRAY,
        "reference_ground_truth": {"type": "object"},
        "emotion_performance_analysis": EMOTION_PERFORMANCE_SCHEMA,
        "reference_fidelity": {
            "type": "object",
            "properties": {
                "camera_geometry": {"type": "string"},
                "subject_geography": {"type": "string"},
                "motion_physics_map": {"type": "string"},
                "action_critical_props": {"type": "string"},
                "hook_mechanics": {"type": "string"},
                "payoff_fidelity": {"type": "string"},
                "must_preserve": {"type": "array", "minItems": 3, "items": {"type": "string"}},
                "forbidden_drift": {"type": "array", "minItems": 2, "items": {"type": "string"}},
            },
            "required": ["camera_geometry", "subject_geography", "motion_physics_map", "action_critical_props", "hook_mechanics", "payoff_fidelity", "must_preserve", "forbidden_drift"],
        },
...
REFERENCE_READABILITY_SCHEMA = {
    "type": "object",
    "properties": {
        "video_readable": {"type": "boolean"},
        "ringkasan_visual": {"type": "string"},
        "subject_roster": PROBE_SUBJECT_ROSTER_SCHEMA,
    },
    "required": ["video_readable", "ringkasan_visual", "subject_roster"],
}
...
def ask(client, prompt, parts=None, json_mode=False, google_search=False, schema=None):
    ...
    config_kwargs = {"temperature": 0.35}
    if json_mode:
        ...
        try:
            ...
            if json_mode:
                ...
                if parsed is not None:
                    ...
                    elif hasattr(parsed, "dict") and callable(parsed.dict):
                        parsed = parsed.dict()
...
def _file_state_name(remote):
    state = getattr(remote, "state", None)
    return str(getattr(state, "name", state or "")).upper()


def _file_processing_error(remote):
    err = getattr(remote, "error", None)
    if err is None:
        return None
    message = getattr(err, "message", None) or getattr(err, "details", None) or str(err)
    return str(message).strip() or None


def upload_to_gemini(client, uploaded_file):
    """Upload media, wait for ACTIVE, and preserve the real server error."""
    if uploaded_file is None:
        return None
    temp_path = None
    try:
        ...
        while True:
            state_name = _file_state_name(remote)
            if state_name == "ACTIVE":
                return remote
            if state_name == "FAILED":
                detail = _file_processing_error(remote)
                raise RuntimeError("Gemini gagal memproses file." + (f" Detail server: {detail}" if detail else ""))
            if time.time() >= deadline:
                raise TimeoutError(f"Gemini masih berstatus {state_name or 'UNKNOWN'} setelah 180 detik.")
            time.sleep(3)
            remote = client.files.get(name=remote.name)
    except Exception as exc:
        st.session_state.reference_processing_error = str(exc)
...
def _transcode_video_for_gemini(uploaded_file):
    """Create a conservative H.264/AAC MP4 fallback when Gemini rejects the source container/codec."""
    if uploaded_file is None:
        return None
    data = uploaded_file.getvalue()
    if not data:
...
def inline_video_part(uploaded_file):
    """Fallback for short videos below Gemini's documented inline limit."""
    if uploaded_file is None:
        return None
    data = uploaded_file.getvalue()
    if not data or len(data) >= 18 * 1024 * 1024:
        return None
    mime = getattr(uploaded_file, "type", None) or "video/mp4"
    st.session_state.reference_input_mode = "inline"
    part = types.Part.from_bytes(data=data, mime_type=mime)
    # Video metadata is optional; avoid constructing a second media payload that
    # can vary across SDK versions. The SDK documents from_bytes() as the
    # canonical local-media path.
    try:
        part.video_metadata = types.VideoMetadata(fps=3.0)
    except Exception:
        pass
    return part
...
def cached_reference_video_part(client, uploaded_file):
    """Reuse a valid cached File; recover with inline video when small."""
    if uploaded_file is None:
        return []
    st.session_state.reference_input_mode = None
    st.session_state.reference_processing_error = None
    st.session_state.reference_readability = {}
    st.session_state.reference_readability_subjects = []
    ...
    if signature and signature in cache:
        ...
        if item.get("name"):
            try:
                remote = client.files.get(name=item["name"])
                if _file_state_name(remote) == "ACTIVE" and remote.uri:
                    st.session_state.reference_input_mode = "files_api_cached"
                    return file_parts_from_remote(remote.uri, remote.mime_type or item.get("mime_type") or "video/mp4")
            except Exception:
                cache.pop(signature, None)
    remote = upload_to_gemini(client, uploaded_file)
    if remote and getattr(remote, "uri", None):
        uri = remote.uri
        mime = getattr(remote, "mime_type", None) or getattr(uploaded_file, "type", None) or "video/mp4"
        if signature:
            cache[signature] = {"name": getattr(remote, "name", None), "uri": uri, "mime_type": mime}
        st.session_state.reference_input_mode = "files_api"
        return file_parts_from_remote(uri, mime)
    # If Files API rejects the original container/codec, try one canonical MP4 transcode.
    # This is deliberately a single fallback transformation; repeated conversions are avoided.
    converted = _transcode_video_for_gemini(uploaded_file)
    if converted and len(converted) < 18 * 1024 * 1024:
        class _ConvertedUpload:
            name = "reference_gemini_compatible.mp4"
            type = "video/mp4"
            def getvalue(self):
                return converted
        remote2 = upload_to_gemini(client, _ConvertedUpload())
        if remote2 and getattr(remote2, "uri", None):
            st.session_state.reference_input_mode = "files_api_transcoded"
            st.session_state.reference_processing_error = None
            return file_parts_from_remote(remote2.uri, remote2.mime_type or "video/mp4")

    fallback = inline_video_part(uploaded_file)
    if fallback is not None:
        # Critical: mark the fallback explicitly so reference_analysis_parts()
        # does NOT append extra keyframes and accidentally inflate the inline request.
        st.session_state.reference_input_mode = "inline"
        st.session_state.reference_processing_error = None
        st.warning("Files API gagal memproses video. Sistem beralih ke Inline Video karena ukuran file di bawah 18 MB.")
        return [fallback]
    return []
...
def reference_analysis_parts(client):
    ...
    if not video_parts:
        return []
    if st.session_state.get("reference_input_mode") == "inline":
        st.session_state.reference_keyframe_timestamps = []
        return list(video_parts)
    keyframes = extract_reference_keyframes(st.session_state.reference_file)
    st.session_state.reference_keyframe_timestamps = [float(item["timestamp"]) for item in keyframes]
...
def invalidate_from_analysis(*, preserve_validation=False, preserve_provenance=False):
...
# ============================================================
def validate_performance_output(data: dict):
    emotion = data.get("emotion_performance_analysis") or {}
    roster = data.get("subject_roster") or []
    profiles = emotion.get("subject_emotion_profiles") or []
    result = validate_performance_profiles(roster, profiles)
    _record_validation("performance_profiles", result)
    if not result["ok"]:
        raise ValueError("Performance profile Gemini tidak lengkap: " + "; ".join(result["issues"][:12]))
    st.session_state["performance_provenance"] = {
        str(x.get("id")): "MODEL_VERIFIED"
        for x in roster if isinstance(x, dict) and x.get("id")
    }
    return True
...
def validate_subject_roster_strict(data: dict) -> list[dict]:
    """Return only a complete Gemini-authored roster; never fabricate missing fields."""
    roster = data.get("subject_roster")
    if not isinstance(roster, list) or not roster:
        raise ValueError("subject_roster harus berupa list non-kosong hasil deteksi Gemini.")
    seen = set()
    required = (
        "id", "jenis", "peran", "deskripsi_visual", "posisi_awal", "orientasi",
        "ekspresi_emosi", "aksi_perilaku", "interaksi_dengan",
    ...
    for i, item in enumerate(roster, 1):
        if not isinstance(item, dict):
            raise ValueError(f"subject_roster item {i} bukan object.")
        missing = [k for k in required if k not in item]
        if missing:
            raise ValueError(f"subject_roster item {i} belum lengkap: {missing}")
        sid = str(item.get("id") or "").strip()
        if not sid:
            raise ValueError(f"subject_roster item {i} memiliki id kosong.")
        if sid in seen:
            raise ValueError(f"subject_roster memiliki duplicate id: {sid}")
        seen.add(sid)
        for key in required:
            value = item.get(key)
            if isinstance(value, list):
                if key == "interaksi_dengan" and not isinstance(value, list):
                    raise ValueError(f"{sid}: interaksi_dengan harus list.")
                if any(not str(x).strip() for x in value):
                    raise ValueError(f"{sid}: {key} mengandung item kosong.")
            elif isinstance(value, bool):
                pass
            elif not str(value).strip():
                raise ValueError(f"{sid}: {key} kosong.")
    return roster


def run_reference_readability_probe(client, parts):
    """Small multimodal gate before the expensive full analysis.

    This separates "Gemini cannot see the video" from "full analysis JSON is
    malformed". It uses the same evidence parts as the real analysis and a tiny
    strict schema.
    """
    probe_prompt = """
Anda adalah video-evidence reader. Baca video/reference yang diberikan, bukan asumsi.
Jawab hanya JSON sesuai schema.

Tugas:
1. Tentukan apakah isi visual video benar-benar dapat dibaca.
2. Ringkas satu kalimat apa yang terlihat.
3. Daftarkan SEMUA subjek yang terlihat dan relevan sepanjang video. Setiap subjek wajib
   memiliki id unik S1, S2, dst., jenis, dan deskripsi visual singkat.
4. Jangan mengarang subjek yang tidak terlihat. Jika video tidak dapat dibaca,
   video_readable=false dan subject_roster boleh kosong.
"""
    raw = ask(client, probe_prompt, parts, json_mode=True, schema=REFERENCE_READABILITY_SCHEMA)
    result = extract_json(raw)
    if not isinstance(result, dict):
        raise ValueError("Probe readability Gemini mengembalikan format yang tidak valid.")
    if result.get("video_readable") is not True:
        raise ValueError(
            "Gemini menerima request tetapi tidak dapat memastikan isi visual video terbaca. "
            "Coba upload ulang video dalam MP4/H.264 atau gunakan screenshot reference."
        )
    probe_roster = result.get("subject_roster", [])
    if not isinstance(probe_roster, list) or not probe_roster:
        raise ValueError("Readability probe tidak menemukan subject roster yang lengkap.")
    seen = set()
    for i, item in enumerate(probe_roster, 1):
        if not isinstance(item, dict) or not str(item.get("id") or "").strip():
            raise ValueError(f"Readability probe subject item {i} tidak memiliki id valid.")
        sid = str(item["id"]).strip()
        if sid in seen:
            raise ValueError(f"Readability probe memiliki duplicate subject id: {sid}")
        seen.add(sid)
    return probe_roster



def _record_validation(stage: str, result: dict, *, hard: bool = True):
    report = st.session_state.setdefault("validation_report", {})
    report[stage] = {"ok": bool(result.get("ok")), "issues": list(result.get("issues", [])), "hard": hard}
    history = st.session_state.setdefault("validation_history", [])
    history.append({"stage": stage, "ok": bool(result.get("ok")), "issues": list(result.get("issues", []))})
    if len(history) > 50:
        del history[:-50]
    return result


def _strict_subject_reconciliation(data: dict):
    probe = st.session_state.get("reference_readability_subjects") or []
    if not probe:
        return {"ok": True, "issues": []}
    result = reconcile_subject_rosters(probe, data.get("subject_roster") or [])
    if result.get("ok"):
        return {"ok": True, "issues": []}
    issues = []
    if result.get("missing"):
        issues.append(f"subject hilang dari full analysis: {result['missing']}")
    if result.get("extra"):
        issues.append(f"subject baru tidak ada di readability probe: {result['extra']}")
    if result.get("conflicts"):
        issues.append(f"subject identity conflict: {result['conflicts']}")
    return {"ok": False, "issues": issues}


def _validate_reference_pipeline(data: dict):
    temporal_result = validate_temporal_crosscheck(data.get("temporal_breakdown") or [], st.session_state.get("detected_reference_duration"))
    _record_validation("temporal_crosscheck", temporal_result)
    if not temporal_result["ok"]:
        raise ValueError("Temporal cross-check gagal: " + "; ".join(temporal_result["issues"][:8]))
    subject_result = _strict_subject_reconciliation(data)
    _record_validation("subject_reconciliation", subject_result)
    if not subject_result["ok"]:
        raise ValueError("Subject reconciliation gagal: " + "; ".join(subject_result["issues"][:8]))
    objects = data.get("object_inventory") or []
    object_ids = [str(x.get("id")) for x in objects if isinstance(x, dict) and x.get("id")]
    object_result = {"ok": len(object_ids) == len(set(object_ids)), "issues": [] if len(object_ids) == len(set(object_ids)) else ["object_inventory memiliki duplicate object ID"]}
    _record_validation("object_identity", object_result)
    if not object_result["ok"]:
        raise ValueError(object_result["issues"][0])
    fidelity_result = validate_reference_fidelity(data.get("reference_fidelity"))
    _record_validation("reference_fidelity_semantic", fidelity_result)
    if not fidelity_result["ok"]:
        raise ValueError("Reference fidelity semantic validation gagal: " + "; ".join(fidelity_result["issues"][:10]))
    keyframes = st.session_state.get("reference_keyframe_timestamps") or []
    if keyframes:
        keyframe_result = validate_temporal_keyframe_alignment(data.get("temporal_breakdown") or [], keyframes)
        _record_validation("temporal_keyframe_alignment", keyframe_result)
        if not keyframe_result["ok"]:
            raise ValueError("Temporal ↔ keyframe evidence gagal: " + "; ".join(keyframe_result["issues"][:8]))
    coverage_result = validate_scene_coverage_complete(data.get("scene_coverage_map") or [], data.get("temporal_breakdown") or [])
    _record_validation("scene_coverage_complete", coverage_result)
    if not coverage_result["ok"]:
        raise ValueError("Scene coverage map tidak lengkap: " + "; ".join(coverage_result["issues"][:10]))
    return True
...
def run_analysis():
    ...
    # Evidence gate: prove the same multimodal payload is visually readable before
    # trusting the large continuity analysis. This prevents a generic roster error
    # from hiding an actual video-ingestion failure.
    try:
        with st.spinner("Menguji apakah Gemini benar-benar dapat membaca isi video..."):
            probe_roster = run_reference_readability_probe(client, parts)
        st.session_state.reference_readability_subjects = probe_roster
        st.session_state.reference_readability = {
            "ok": True,
            "subject_count": len(probe_roster),
            "subject_ids": [x["id"] for x in probe_roster],
        }
    except Exception as exc:
        st.session_state.reference_readability = {"ok": False, "error": str(exc)}
        st.error(f"Readability gate gagal: {exc}")
        return
    ...
    if detected is not None:
        ...
        if min_label and chosen_seconds + 0.25 < detected:
            raise ValueError(
                f"Durasi target ({chosen_seconds:.2f}s) lebih pendek dari reference ({detected:.2f}s). "
                f"Pilih minimal {min_label} sebelum menjalankan analisis."
            )
...
"""
    ...
    with st.spinner("Membedah referensi penuh: timeline, seat map, kamera, geography, motion physics, prop state, performance, hook, dan payoff..."):
        try:
            ...
            validate_subject_roster_strict(data)
            ...
            beats = data.get("temporal_breakdown")
            if not isinstance(beats, list) or not beats:
                raise ValueError("temporal_breakdown harus berupa list beat yang tidak kosong.")
            invalid_beats = []
            for idx, beat in enumerate(beats, 1):
                if not isinstance(beat, dict):
                    invalid_beats.append(idx)
                    continue
                required_fields = ("beat", "waktu", "start_state", "cause", "action", "end_state")
                if any(not str(beat.get(key, "")).strip() for key in required_fields):
                    invalid_beats.append(idx)
            if invalid_beats:
                raise ValueError(f"temporal_breakdown memiliki beat kosong/tidak lengkap: {invalid_beats}.")
            validate_performance_output(data)
            verified_performance_provenance = deepcopy(st.session_state.get("performance_provenance", {}))
            ...
            _validate_reference_pipeline(data)
            data["karakter_utama"] = deepcopy(CHARACTER_LOCK)
            saved_validation_report = deepcopy(st.session_state.get("validation_report", {}))
            saved_validation_history = deepcopy(st.session_state.get("validation_history", []))
            st.session_state.analysis = data
            st.session_state.character = deepcopy(CHARACTER_LOCK)
            invalidate_from_analysis(preserve_validation=True, preserve_provenance=True)
            st.session_state.validation_report = saved_validation_report
            st.session_state.validation_history = saved_validation_history
            # Keep verified analysis artifacts after clearing downstream generation artifacts.
            st.session_state.emotion_performance = deepcopy(data.get("emotion_performance_analysis", {}))
            st.session_state.performance_provenance = verified_performance_provenance
...
def render_analysis():
    ...
    if validation:
        failed = [k for k, v in validation.items() if not v.get("ok")]
        if failed:
            st.error(f"Validation Gate: {len(failed)} gate gagal — pipeline dikunci.")
        else:
            st.success(f"Validation Gate: {len(validation)} gate lolos.")
...
def generate_scene_contract(scene_number):
    ...
    with st.spinner(f"Menyusun contract Scene {scene_number}..."):
        try:
            ...
            coverage_result = validate_scene_coverage(scene, temporal_beats_for_scene(scene_number))
            _record_validation(f"scene_{scene_number}_coverage", coverage_result)
            if not coverage_result["ok"]:
                raise ValueError(f"Scene {scene_number} temporal coverage gagal: {coverage_result['issues']}")
            causal_result = validate_causal_integrity(scene)
            _record_validation(f"scene_{scene_number}_causal", causal_result)
            if not causal_result["ok"]:
                raise ValueError(f"Scene {scene_number} causal integrity gagal: {causal_result['issues']}")
            if scene_number > 1:
                previous_contract = st.session_state.storyboard[scene_number - 2] if len(st.session_state.storyboard) >= scene_number - 1 else {}
                transition_result = validate_scene_transition(previous_contract, scene)
                _record_validation(f"scene_{scene_number}_transition", transition_result)
                if not transition_result["ok"]:
                    raise ValueError(f"Scene {scene_number} END→START reconciliation gagal: {transition_result['issues']}")
...
"""
    with st.spinner(f"Membuat prompt Scene {scene_number}..."):
        try:
            ...
            prompt_check = validate_prompt_against_contract(result, scene)
            _record_validation(f"scene_{scene_number}_prompt_contract", prompt_check)
            if not prompt_check["ok"]:
                raise ValueError(f"Prompt bertentangan dengan scene contract: {prompt_check['issues']}")
