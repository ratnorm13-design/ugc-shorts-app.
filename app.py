...
APP_VERSION = "15.1 — Video File API + Agentic Analysis + Strict Workflow Gates"
...
INLINE_VIDEO_FALLBACK_MB = 20
MAX_SUPPORTED_SECONDS = 300
FILE_API_POLL_SECONDS = 2
FILE_API_TIMEOUT_SECONDS = 90
...
MAP_OPTIONS = [
    ...
    "Colorful indoor game arena",
    "Coastal boardwalk game map",
    "Neon fantasy game city",
    "Soft-foam obstacle arena",
    "Bright toy-like game stadium",
]
...
CLIMAX_ACTION_OPTIONS = [
    ...
    "Soft bumper contact followed by an exaggerated ragdoll bounce",
    "Spring-pad bounce followed by a cartoon ragdoll tumble",
    "Double foam-bumper contact followed by a cartoon bounce",
    "Soft game-pad sequence followed by a harmless ragdoll tumble",
    "Bouncy game-platform contact followed by a soft physics tumble",
    "Foam-gate collision followed by an exaggerated ragdoll reaction",
]
...
MANEUVER_OPTIONS = [
    ...
    "Step around a soft moving prop",
    "Low duck under a foam gate",
    "Spring-pad bounce",
    "Balance across a wide game platform",
    "Quick taunt emote while running",
    "Bouncy conveyor traversal",
    "Duck under a soft moving prop",
    "Speed-pad acceleration",
    "Hop across wide foam pads",
]
...
DEFAULTS = {
    ...
    "reference_name": "",
    "reference_mime": "",
    "reference_upload_signature": "",
    "reference_file_uri": "",
    "reference_file_name": "",
    "reference_file_state": "",
    "reference_input_mode": "",
    "reference_duration_source": "",
    "reference_text": "",
...
def _get_api_key() -> str:
    ...
    if not key:
        try:
            ...
            key = ""
    if not key:
        key = str(st.session_state.get("api_key", ""))
...
def get_client() -> genai.Client:
    ...
    if not key:
        raise RuntimeError(
            "Gemini API Key belum diisi. Masukkan Auth/API key di sidebar atau Streamlit Secrets."
        )
    try:
...
def _is_model_unavailable(message: str) -> bool:
    ...
    return any(token in upper for token in (
        "NOT_FOUND", "MODEL_NOT_FOUND", "404", "NOT SUPPORTED", "UNSUPPORTED MODEL"
    ))


def _is_quota_error(message: str) -> bool:
    upper = message.upper()
    return any(token in upper for token in ("RESOURCE_EXHAUSTED", "QUOTA", "429"))


def _is_auth_error(message: str) -> bool:
    upper = message.upper()
    return any(token in upper for token in (
        "API_KEY_INVALID", "INVALID_API_KEY", "UNAUTHENTICATED",
...
def _is_invalid_request(message: str) -> bool:
    upper = message.upper()
    return any(token in upper for token in ("INVALID_ARGUMENT", "400", "FAILED_PRECONDITION"))


def _is_transient(message: str) -> bool:
    upper = message.upper()
    return any(token in upper for token in ("500", "502", "503", "504", "UNAVAILABLE", "INTERNAL"))
...
def ask(
    ...
    """Single controlled Gemini request with model failover.

    A 503 on one model is not treated as an authentication failure and does not
    abort the whole workflow. Each model gets at most one transient retry; then
    the router moves to the next model. Quota/auth/invalid-request errors stop
    immediately because another model will not fix a project-level problem.
    """
    client = get_client()
        ...
        for attempt in range(2):
            try:
                ...
                if _is_quota_error(message):
                    raise RuntimeError(
                        "Kuota/rate limit Gemini tercapai. Tidak dilakukan retry lintas model."
                    ) from exc
    ...
    raise RuntimeError(
        "Semua model Gemini yang dikonfigurasi gagal/temporer tidak tersedia. "
        "Tidak ada loop retry tanpa batas.\n\n" + "\n".join(errors[-8:])
    )
...
# VIDEO INPUT
# ============================================================
def _probe_video_duration(data: bytes, suffix: str) -> float | None:
    """Read duration from ffprobe; never ask Gemini to guess file duration."""
    path = None
    try:
...
def _inline_video_part() -> list:
    data = st.session_state.get("reference_bytes")
    if not data:
        return []
    if len(data) > INLINE_VIDEO_FALLBACK_MB * 1024 * 1024:
        raise ValueError(
            f"Inline fallback hanya untuk video <= {INLINE_VIDEO_FALLBACK_MB} MB. "
            "File API upload gagal dan file terlalu besar untuk fallback inline."
        )
...
def _upload_reference_file(client):
    data = st.session_state.get("reference_bytes")
    if not data:
        return None

    signature = hashlib.sha256(data).hexdigest()
    cached_signature = st.session_state.get("reference_upload_signature", "")
    cached_uri = st.session_state.get("reference_file_uri", "")
    if cached_signature == signature and cached_uri:
        try:
            cached = client.files.get(name=st.session_state.reference_file_name)
            if _file_state_name(cached) == "ACTIVE":
                return cached
        except Exception:
            pass

    suffix = os.path.splitext(st.session_state.get("reference_name", "reference.mp4"))[1] or ".mp4"
    tmp_path = None
    try:
        ...
        config = types.UploadFileConfig(
            display_name=st.session_state.get("reference_name") or "reference-video",
            mime_type=st.session_state.get("reference_mime") or "video/mp4",
        )
        remote = client.files.upload(file=tmp_path, config=config)

        deadline = time.time() + FILE_API_TIMEOUT_SECONDS
        while True:
            state_name = _file_state_name(remote)
            if state_name == "ACTIVE":
                break
            if state_name in {"FAILED", "ERROR"}:
                error = getattr(remote, "error", None)
                raise RuntimeError(f"Gemini File API gagal memproses video: {error or state_name}")
            if time.time() >= deadline:
                raise TimeoutError("Gemini File API terlalu lama memproses video (>90 detik).")
            time.sleep(FILE_API_POLL_SECONDS)
            remote = client.files.get(name=remote.name)

        st.session_state.reference_upload_signature = signature
        st.session_state.reference_file_uri = remote.uri
        st.session_state.reference_file_name = remote.name
        st.session_state.reference_file_state = "ACTIVE"
...
def get_reference_parts() -> list:
    ...
    if data:
        client = get_client()
        try:
            remote = _upload_reference_file(client)
            return [
                types.Part.from_uri(
                    file_uri=remote.uri,
                    mime_type=remote.mime_type or st.session_state.get("reference_mime") or "video/mp4",
                    media_processing="AGENTIC",
                )
            ]
        except Exception as exc:
            message = _error_text(exc)
            # Auth/quota errors must never be hidden by a media fallback.
            if _is_auth_error(message) or _is_quota_error(message):
                raise
            if len(data) <= INLINE_VIDEO_FALLBACK_MB * 1024 * 1024:
                st.warning(
                    "Gemini File API tidak bisa dipakai untuk video ini; menggunakan "
                    "inline video fallback karena file masih <= 20 MB."
                )
                st.session_state.reference_input_mode = "inline_fallback"
                return _inline_video_part()
            raise

    text = st.session_state.get("reference_text", "").strip()
    if text:
        st.session_state.reference_input_mode = "text"
...
def _workflow_signature() -> str:
    payload = {
        "reference_sha": hashlib.sha256(st.session_state.get("reference_bytes") or b"").hexdigest(),
        "reference_text": st.session_state.get("reference_text", ""),
        "visual_style": st.session_state.get("visual_style"),
        "selected_camera": st.session_state.get("selected_camera"),
        "runner_choice": st.session_state.get("runner_choice"),
        "custom_runner": st.session_state.get("custom_runner"),
        "target_doll_choice": st.session_state.get("target_doll_choice"),
        "target_idle_choice": st.session_state.get("target_idle_choice"),
        "selected_map": st.session_state.get("selected_map"),
        "selected_prop_stand": st.session_state.get("selected_prop_stand"),
        "selected_climax_action": st.session_state.get("selected_climax_action"),
        "aspect_ratio": st.session_state.get("aspect_ratio"),
        "duration": st.session_state.get("duration"),
        "obstacles": st.session_state.get("user_scene_obstacles", {}),
        "maneuvers": st.session_state.get("user_scene_maneuvers", {}),
    }
    raw = json.dumps(payload, ensure_ascii=False, sort_keys=True, default=str)
    return hashlib.sha256(raw.encode("utf-8")).hexdigest()


def _workflow_is_stale() -> bool:
    stored = st.session_state.get("analysis_signature", "")
    return bool(st.session_state.get("analysis")) and stored != _workflow_signature()
...
def run_analysis() -> None:
    ...
    if st.session_state.get("reference_bytes"):
        probed_duration = _probe_video_duration(
            st.session_state.reference_bytes,
            os.path.splitext(st.session_state.get("reference_name", "reference.mp4"))[1],
        )
        if probed_duration is not None:
            st.session_state.detected_duration_seconds = probed_duration
            st.session_state.reference_duration_source = "ffprobe"
...
Scene-specific instructions are supplied later during sequential scene generation.

Return JSON only:
{{
  "video_duration_seconds": {round(probed_duration, 3) if probed_duration is not None else 16},
  "original_reference": {{
    "runner_asli": "...",
    "boss_asli": "...",
    "track_asli": "..."
...
- If a reference video duration is supplied by the system, treat it as ground truth and do not invent another duration.
...
"""
    ...
    with st.spinner("Menganalisis referensi dengan Gemini..."):
        try:
            ...
            raw_seconds = float(probed_duration if probed_duration is not None else data["video_duration_seconds"])
            if raw_seconds <= 0:
                raise ValueError("Durasi video tidak berhasil ditentukan. Pastikan video valid atau gunakan deskripsi teks.")
            ...
            if probed_duration is None:
                st.session_state.reference_duration_source = "model_estimate"
            ...
            st.session_state.analysis_signature = _workflow_signature()
            ...
            st.session_state.scene_instructions = {}
...
"""
    ...
    scene_instruction = st.session_state.scene_instructions.get(scene_number, "").strip()
...
Scene-specific extra instruction: {scene_instruction or "none"}
{final_note}
...
def render_home() -> None:
    ...
    if uploaded is not None:
        ...
        if len(data) > MAX_FILE_SIZE_MB * 1024 * 1024:
            ...
            st.session_state.reference_upload_signature = ""
            st.session_state.reference_file_uri = ""
            st.session_state.reference_file_name = ""
            st.session_state.reference_file_state = ""
            st.session_state.reference_input_mode = ""
...
def render_analysis() -> None:
    ...
    st.success(
        ...
        f"Media: **{st.session_state.get('reference_input_mode') or '-'}**"
    )

    duration = st.session_state.get("detected_duration_seconds")
    if duration:
        source = st.session_state.get("reference_duration_source") or "unknown"
        st.write(f"Reference duration: **{duration:.1f} detik** · source: **{source}**")
...
def render_scenes() -> None:
    if not st.session_state.get("analysis"):
        st.warning("Scene terkunci. Selesaikan ANALYZE + AUTO REMIX terlebih dahulu.")
        return
    if _workflow_is_stale():
        st.warning("Konfigurasi project berubah setelah analisis. Jalankan ANALYZE + AUTO REMIX lagi sebelum membuat scene.")
        return

    total = scene_count()
    current = max(1, min(int(st.session_state.current_scene), total))
    ...
    if current not in st.session_state.scene_prompts:
        scene_instruction_value = st.text_area(
            f"Instruksi Tambahan — Scene {current} (hanya berlaku untuk scene ini)",
            value=st.session_state.scene_instructions.get(current, ""),
            height=90,
            key=f"scene_instruction_input_{current}",
        )
        st.session_state.scene_instructions[current] = scene_instruction_value.strip()
...
def generate_seo() -> None:
    total = scene_count()
    if not st.session_state.get("analysis"):
        st.error("SEO terkunci: belum ada hasil analisis.")
        return
    if _workflow_is_stale():
        st.error("SEO terkunci: konfigurasi berubah setelah analisis. Jalankan analisis ulang.")
        return
    if not all(i in st.session_state.scene_prompts for i in range(1, total + 1)):
        st.error("SEO terkunci: semua prompt scene harus selesai terlebih dahulu.")
        return
    if not all(i in st.session_state.scene_frames for i in range(1, total)):
        st.error("SEO terkunci: visual bridge setiap scene sebelumnya harus tersedia.")
        return
...
def render_seo() -> None:
    total = scene_count()
    if not st.session_state.get("analysis"):
        st.warning("SEO terkunci sampai analisis selesai.")
        return
    if _workflow_is_stale():
        st.warning("SEO terkunci karena konfigurasi berubah setelah analisis. Jalankan analisis ulang.")
        return
    if not all(i in st.session_state.scene_prompts for i in range(1, total + 1)):
        st.warning("SEO terkunci sampai semua prompt scene selesai.")
        return
    if not all(i in st.session_state.scene_frames for i in range(1, total)):
        st.warning("SEO terkunci sampai semua visual bridge selesai.")
        return
...
with st.sidebar:
    ...
    if st.session_state.get("analysis"):
        if st.button("🔍 Analysis", use_container_width=True):
            go("analysis")
        if not _workflow_is_stale():
            if st.button("🎥 Scenes", use_container_width=True):
                go("scenes")
        else:
            st.caption("🔒 Scenes terkunci karena konfigurasi berubah; analisis ulang diperlukan.")
    else:
        st.caption("🔒 Analysis terkunci sampai reference dianalisis.")

    all_scene_prompts = bool(st.session_state.get("analysis")) and not _workflow_is_stale() and all(
        i in st.session_state.scene_prompts for i in range(1, scene_count() + 1)
    )
    all_bridges = all(
        i in st.session_state.scene_frames for i in range(1, scene_count())
    ) if scene_count() > 1 else True
    if all_scene_prompts and all_bridges:
        if st.button("🚀 SEO", use_container_width=True):
            go("seo")
    else:
        st.caption("🔒 SEO terbuka setelah semua scene + visual bridge selesai.")
