...
16. PEMBAGIAN DURASI: 16 detik = 2 scene, 24 detik = 3 scene, 32 detik = 4 scene, dan seterusnya sesuai mapping durasi. Setiap scene sekitar 8 detik. Jumlah scene hanya membagi waktu; jangan membuat plot baru untuk memenuhi jumlah scene.
17. CONTINUITY BERANTAI: Scene 1 mulai dari START STATE referensi. Untuk setiap Scene N setelah itu, START STATE harus sama dengan END STATE Scene N-1. Setelah ACTION selesai, tulis END STATE yang konkret dan dapat langsung menjadi START STATE scene berikutnya. Aturan ini berlaku terus sampai scene terakhir, termasuk Scene 3, 4, 5, dan seterusnya.
18. Jika satu beat referensi membutuhkan beberapa scene, pecah menjadi langkah-langkah mikro yang benar-benar terjadi secara natural dan tetap mempertahankan sebab-akibat. Jangan melompati aksi penting hanya karena batas 8 detik.
...
14. CHAIN RULE: Scene 3, Scene 4, dan semua scene setelahnya mengikuti pola yang sama: START STATE = END STATE scene sebelumnya -> CAUSE -> ACTION -> END STATE. Jangan pernah memperlakukan scene sebagai adegan mandiri.
15. Jumlah scene hanya pembagian durasi sekitar 8 detik per scene; alur tidak boleh melompat hanya karena pergantian scene.
...
def generate_scene_prompt(scene_number):
    ...
    prompt = f"""
Write ONE production-ready Google Flow / Veo prompt in ENGLISH for Scene {scene_number}, {part_label}.
